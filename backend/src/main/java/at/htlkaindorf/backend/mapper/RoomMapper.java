...
public interface RoomMapper {
    ...
    @Mapping(target = "assignedObjectIds", expression = "java(mapAssignedObjectIds(room.getAssignedObjects()))")
    ...
    default List<String> mapAssignedObjectIds(List<ObjectDocument> objects) {
        if (objects == null) return List.of();
        return objects.stream()
                .map(obj -> obj != null ? obj.getId() : null)
                .filter(id -> id != null)
