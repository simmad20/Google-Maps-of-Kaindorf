...
public class RoomService {
    ...
    public RoomDetailedDTO getRoomWithObjectsByType(String roomId, String objectType) {
        Room room = roomRepository.findById(roomId)
                .orElseThrow(() -> new IllegalArgumentException("Raum nicht gefunden: " + roomId));
        ...
        List<ObjectDocument> filteredObjects = room.getAssignedObjects().stream()
                .filter(obj -> objectType.equals(obj.getType()))
                .collect(Collectors.toList());
    ...
    public RoomDTO createRoom(CreateRoomRequestDTO request, String cardId) {
        ...
        room.setCard(cardRepository.findCardById(cardId).orElseThrow(() -> new IllegalArgumentException("Karte nicht gefunden: " + cardId)));
    ...
    @Transactional
    public void removeObjectFromRoom(String roomId, String objectId, String objectType) {
        Room room = roomRepository.findById(roomId)
                .orElseThrow(() -> new IllegalArgumentException("Raum nicht gefunden: " + roomId));

        boolean removed = room.getAssignedObjects().removeIf(obj ->
                obj.getId().equals(objectId) && objectType.equals(obj.getType()));
        ...
        ObjectDocument object = objectRepository.findByIdAndType(objectId, objectType)
    ...
    public List<ObjectDocument> getObjectsInRoomByType(String roomId, String objectType) {
