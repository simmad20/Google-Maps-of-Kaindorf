...
public class DataInitializer {
    ...
    @PostConstruct
    public void init() {
        // ObjectType erstellen falls nicht vorhanden
        if (objectTypeRepository.findByDisplayName("teacher").isEmpty()) {
            ObjectType teacherType = ObjectType.builder()
                    .displayName("teacher")
                    .allowedAttributes(Arrays.asList("firstname", "lastname", "abbreviation", "image_url", "title"))
                    .build();
            objectTypeRepository.save(teacherType);
        }
        if (objectRepository.count() == 0) {
            migrateTeachersFromJson();
        }
        /*if (cardRepository.count() == 0) {
            Card ogCard = Card.builder()
                    .title("OG")
                    .build();

            ogCard = cardRepository.save(ogCard);
            Card finalOgCard = ogCard;
            List<Room> rooms = roomRepository.findAll().stream().peek(room -> room.setCard(finalOgCard)).toList();
            roomRepository.saveAll(rooms);
            ogCard.setRooms(rooms);
            cardRepository.save(ogCard);
        }*/
    }
